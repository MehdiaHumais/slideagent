    # SLIDE 6: FINANCIAL PROJECTIONS ENFORCEMENT
        if "Financial" in slide["title"] or "Projection" in slide["title"] or "Model" in slide["title"]:
            print(f"[*] Audit Protection: Enforcing Million 'M' Scaling on Slide 6.")
            slide["title"] = "FINANCIAL MODEL: 3-YEAR REVENUE PROJECTIONS"
            slide["bullets"] = [
                "Year 1 Target: GBP 1.01M (Pilot Phase Growth)",
                "Year 2 Scale: GBP 2.42M (Continental Expansion)",
                "Year 3 Maturity: GBP 4.95M (Global Market Integration)"
            ]
            slide["type"] = "table"
...
def add_fade_transition(pptx_path):
    """Applies a smooth fade transition to all slides using XML injection."""
    if not etree: return
    try:
        with zipfile.ZipFile(pptx_path, 'a') as z:
            for name in z.namelist():
                if name.startswith('ppt/slides/slide') and name.endswith('.xml'):
                    ...
                    sld_nm = '{http://schemas.openxmlformats.org/presentationml/2006/main}'
                    
                    # Remove existing transition if any
                    for transition in root.xpath('//p:transition', namespaces={'p': 'http://schemas.openxmlformats.org/presentationml/2006/main'}):
                        transition.getparent().remove(transition)
                        
                    # Add new fade transition
                    trans = etree.SubElement(root, f'{sld_nm}transition', spd='slow')
                    etree.SubElement(trans, f'{sld_nm}fade')
                    z.writestr(name, etree.tostring(root))
    except Exception as e:
        print(f"[!] Transition failed: {e}")

# ==========================================
# CORE UTILITIES (SHAPES & TEXT)
# ==========================================
...
def add_shape(slide, color, left, top, width, height, transparency=0.0):
...
def add_textbox(slide, left, top, width, height, text, font_size=18, color=RGBColor(0,0,0), bold=False, align=PP_ALIGN.LEFT, font_name="Arial"):
...
def extract_financial_data(text):
    """Heuristic extraction of budget and pillars for validation."""
    data = {}
    budget_match = re.search(r'(?:GBP|£|\$)?\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*(?:budget|investment|requirement|funding)', text, re.I)
    if budget_match: data['budget_total'] = budget_match.group(1)
    
    pillars = []
    # Identify pillars (Section 5 typically)
    pillar_matches = re.finditer(r'Pillar\s*(\d+):?\s*([^\n.]+)', text, re.I)
    for m in pillar_matches:
        pillars.append(m.group(2).strip())
    if pillars: data['revenue_pillars'] = pillars
